...
impl Target {
    pub fn new(extra_derives: &str) -> Self {
        let derive_str: String = if extra_derives.is_empty() {
                "".to_string()
              }
              else {
                extra_derives.split(',').map(|s| format!(", {}",s.trim())).collect()
              };

        Self {
          derive_str: format!("#[derive(Serialize, Deserialize{})]", derive_str)
        }
    }
}
    ...
    fn item(
        ...
        Ok(match item {
            ...
            target::Item::Preamble => {
                writeln!(
                    out,
                    "// Code generated by jtd-codegen, custom build, for Rust v{}",
